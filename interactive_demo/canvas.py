...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas, main_app, model_input_type=IType.point):
        ...
        self.canvas.bind('<B1-Motion>', self.__left_mouse_button_motion)
    ...
    def __left_mouse_button_pressed(self, event):
        ...
        if coords is not None:
            self._callback(status=IStatus.begin, is_positive=True, x=coords[0], y=coords[1])
    ...
    def __left_mouse_button_released(self, event):
        ...
        coords = self._get_click_coordinates(event)
        if coords is not None:
            self._callback(status=IStatus.end)

    def __left_mouse_button_motion(self, event):
        coords = self._get_click_coordinates(event)
        if coords is not None:
            self._callback(status=IStatus.motion, x=coords[0], y=coords[1])
            self.main_app._update_image()
    ...
    def __right_mouse_button_pressed(self, event):
        ...
        coords = self._get_click_coordinates(event)
        if coords is not None:
            self._callback(status=IStatus.begin, is_positive=False, x=coords[0], y=coords[1])
