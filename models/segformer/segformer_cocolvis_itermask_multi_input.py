...
MODEL_NAME = 'cocolvis_segformer_multi'
...
def init_model(cfg):
    ...
    model = SegFormerModel(
        ...
        is_training=True,
        input_type=[IType.point, IType.contour, IType.stroke],
        accumulate_multi_input=cfg.MODEL.ACCUMULATE_MULTI_INPUT,
    )
    model.to(cfg.device)
...
def train(model, cfg, model_cfg):
    ...
    interactive_info_sampler = ComposeInteractionSampler({
        ...
        IType.stroke: MultiStrokesSampler(
    ...
    trainer = ISTrainer(
        ...
        trainset, valset,
        optimizer=cfg.MODEL.OPTIMIZER_TYPE, optimizer_params=optimizer_params,
        ...
        max_interactive_points=model_cfg.max_objects, max_num_next_interactions=max_num_next_interactions,
        train_itype_selector=RandomInteractionSelector(),
        val_itype_selector=InteractionSelector(),
    )
