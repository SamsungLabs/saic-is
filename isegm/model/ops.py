...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, rows, cols):
        ...
        coords, coords_idx = coords.min(dim=1)  # -> (bs * num_masks * 2) x 1 x h x w
...
class ContoursMaps(nn.Module):
    def __init__(self):
        super(ContoursMaps, self).__init__()

    def forward(self, x, contours):
        return contours


class StrokesMaps(nn.Module):
    def __init__(self):
        super(StrokesMaps, self).__init__()
