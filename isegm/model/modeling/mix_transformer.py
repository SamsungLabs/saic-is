...
class Attention(nn.Module):
    ...
    def forward(self, x, height, width):
        batch_size, n_keys, n_ch = x.shape
        q = self.q(x).reshape(batch_size, n_keys, self.num_heads, n_ch // self.num_heads).permute(0, 2, 1, 3)

        if self.sr_ratio > 1:
            img_x = x
            img_x = img_x.permute(0, 2, 1).reshape(batch_size, n_ch, height, width)
            img_x = self.sr(img_x).reshape(batch_size, n_ch, -1).permute(0, 2, 1)
            all_x = self.norm(img_x)
            kv = self.kv(all_x).reshape(
                batch_size, -1, 2, self.num_heads, n_ch // self.num_heads
            ).permute(2, 0, 3, 1, 4)
        else:
            kv = self.kv(x).reshape(batch_size, -1, 2, self.num_heads, n_ch // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        ...
        x = (attn @ v).transpose(1, 2).reshape(batch_size, n_keys, n_ch)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class InteractionAsImageEmbed(nn.Module):
    ...
    def forward(self, coord_features):
        x, _, _ = self.patch_embed1(coord_features)
        return x
...
class MixVisionTransformer(nn.Module):
    def __init__(
        ...
        self.with_prev_mask = with_prev_mask
        img_size_tuple = img_size if isinstance(img_size, tuple) or isinstance(img_size, list) else (img_size, img_size)

        self.patch_embed1 = OverlapPatchEmbed(
            img_size=img_size_tuple, patch_size=7, stride=4, in_chans=in_chans, embed_dim=embed_dims[0]
        )
        self.patch_embed2 = OverlapPatchEmbed(
            img_size=(img_size_tuple[0] // 4, img_size_tuple[1] // 4),
            patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1]
        )
        self.patch_embed3 = OverlapPatchEmbed(
            img_size=(img_size_tuple[0] // 8, img_size_tuple[1] // 8),
            patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2]
        )
        self.patch_embed4 = OverlapPatchEmbed(
            img_size=(img_size_tuple[0] // 16, img_size_tuple[1] // 16),
    ...
    def forward_features(self, x, coord_features=None):
    ...
    def forward(self, x, coord_features=None):
        x = self.forward_features(x, coord_features=coord_features)
        return x
