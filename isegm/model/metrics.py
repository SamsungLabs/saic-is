...
class AdaptiveIoU(TrainMetric):
    def __init__(
        self, init_thresh=0.4, thresh_step=0.025, thresh_beta=0.99, iou_beta=0.9,
...
class Accuracy(TrainMetric):
    def __init__(self, ignore_label=-1, from_logits=True, pred_output='instances', gt_output='instances'):
        super().__init__(pred_outputs=(pred_output,), gt_outputs=(gt_output,))
        self._ignore_label = ignore_label
        self._from_logits = from_logits
        self._epoch_acc_sum = 0.0
        self._epoch_batch_count = 0
    ...
    def update(self, pred, gt):
        ...
        if self._from_logits:
            pred = torch.sigmoid(pred)
        pred = pred > 0.5
        acc = torch.sum(gt_mask == pred).detach().cpu().numpy() / np.prod(gt.shape)
        self._epoch_acc_sum += acc
        self._epoch_batch_count += 1

    def get_epoch_value(self):
        if self._epoch_batch_count > 0:
            return self._epoch_acc_sum / self._epoch_batch_count
        else:
            return 0.0
    ...
    def log_states(self, sw, tag_prefix, global_step):
        # Add Accuracy_iter to tensorboard
        sw.add_scalar(tag=tag_prefix + '_iter', value=self.get_epoch_value(), global_step=global_step)
