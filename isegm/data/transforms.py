...
try:
    ...
    from albumentations.augmentations.functional import resize, keypoint_scale
...
class UniformRandomResize(DualTransform):
    def __init__(self, scale_range=(0.9, 1.1), interpolation=cv2.INTER_LINEAR, always_apply=False, p=1, input_type=IType.point):
...
class ZoomIn(DualTransform):
    ...
    def apply_to_mask(self, mask, selected_object, bbox, **params):
        if selected_object is None:
            if self.always_resize:
                mask = resize(mask, height=self.height, width=self.width,
        ...
        new_mask = resize(new_mask, height=self.height, width=self.width,
                            interpolation=cv2.INTER_NEAREST)
        return new_mask
