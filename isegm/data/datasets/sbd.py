...
class SBDDataset(ISDataset):
    ...
    def get_sample(self, index):
        ...
        return DSample(
            image, instances_mask,
            objects_ids=instances_ids, sample_id=index,
...
class SBDEvaluationDataset(ISDataset):
    ...
    def get_sbd_images_and_ids_list(self):
        ...
        bad_examples = []
        with open(self.dataset_path / "sbd_val_bad_lst.txt", "r") as f:
            for line in f:
                k, v = line.split()
                bad_examples.append((k, int(v)))

        if pkl_path.exists():
            with open(str(pkl_path), 'rb') as fp:
                images_and_ids_list = pkl.load(fp)
            images_and_ids_list = [elem for elem in images_and_ids_list if elem not in bad_examples]
        else:
            images_and_ids_list = []
            ...
            for sample in self.dataset_samples:
                ...
                for instances_id in instances_ids:
                    if sample in bad_examples and instances_id in bad_examples[sample]:
                        continue
